...
# Второй уровень
### Задание
1. Сделать свою программу выполняющейся до тех пор, пока пользователь не введёт все нулевые значения.
2. Реализовать сохранение вывода программы и пользовательского ввода в лог-файл.

### Первый пункт
для того чтобы выполнить первый пункт, я поместил код тела программы в цикл do while с условием, что хотябы одно из значений не равно 0.

### Второй пункт
Для реализации второго пункта, я пользуюсь следующими командами:
1. `FILE *x;` - Обьявляю файловую переменную `x`
2. `x=fopen("loginto.txt","r");` - присваиваю файловой переменной `x` значение `отктрыть loginto.txt` `открыть для чтения`
3. `fscanf(x,"%d",&y);` - читаю число из переменной `x` и записываю это число в файловую переменную `y`
4. `fprintf(x,"%f\n", n);` - вывожу в файловую переменную `x` \
![](ScrSec1.png)   ![](ScrSec2.png)

### Запуск файла nsl.c
(Программа считывает значения из файла loginto.txt и записывает результат в файл logout.txt)
1. Открыть папку XRevLab1 с помощью VSCodium 
2. Прописать в консоли `gcc nsl.c -o Result`
